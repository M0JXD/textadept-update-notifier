...
--- Conditional for `test.mock(os, 'spawn')` if the notifier is requesting release information.
local function is_request(cmd) return cmd:sub(1, #update_notifier.fetch) == update_notifier.fetch end
...
test('update_notifier.check show show stable -> stable update', function()
...
test('update_notifier.check should view release if selected', function()
	local current, next_beta, next_stable = '1.0', '2.0 beta', '1.1'
	local _<close> = test.mock(_G, '_RELEASE', 'Textadept ' .. current)
	local _<close> = test.mock(os, 'spawn', is_request, mock_spawn(next_beta, next_stable))
...
test('update_notifier.check should show beta -> beta update', function()
...
test('update_notifier.check should show beta -> stable update', function()
...
test('update_notifier.check should not show stable -> beta update', function()
...
test('update_notifier.check should show update for nightly -> any new release', function()
...
test('update_notifier.check should not show update for nightly -> prior release', function()
	local current, next_beta, current_stable = '1.1 nightly', '1.1 beta', '1.0'
	local _<close> = test.mock(_G, '_RELEASE', current)
	local message = test.stub()
	local _<close> = test.mock(ui.dialogs, 'message', message)
